import { take, fold as fold_1, toList, head, sortBy } from "../fable-library-js.4.21.0/Seq.js";
import { equals, defaultOf, round, comparePrimitives } from "../fable-library-js.4.21.0/Util.js";
import { Record, Union } from "../fable-library-js.4.21.0/Types.js";
import { float64_type, record_type, bool_type, string_type, class_type, union_type } from "../fable-library-js.4.21.0/Reflection.js";
...
export function DataSimulation_$reflection() {
    return class_type("Sutil.DataSimulation", undefined, DataSimulation);
}

export function DataSimulation_Stream(init, f, delay) {
    let dispose = defaultOf();
    let tick = 0;
...
export function DataSimulation_CountList_4F7761DC(min, max, delay) {
    ...
    return DataSimulation_Stream(empty, (t, current) => {
        const i = (t % n) | 0;
        if (i === 0) {
            return singleton(min);
        }
        else {
            return append(current, singleton(min + i));
        }
    }, delay);
}
...
export function DataSimulation_Records_5F9D7765(data, update, delay) {
    return DataSimulation_Records_Z46B86D6D(data, update, 1, true, delay);
}

export function DataSimulation_Count_4F7761DC(min, max, delay) {
    return DataSimulation_Stream(() => min, (_arg_1, n) => {
        if (n === max) {
            return min | 0;
        }
        else {
...
export function SampleData_Todo_$reflection() {
...
export function SampleData_Name_$reflection() {
...
export function SampleData_Stock_$reflection() {
...
export function SampleData_stockFeed(numStocks, delay) {
    return DataSimulation_Records_Z46B86D6D(SampleData_sampleStocks(numStocks), SampleData_updateStock, 5, false, delay);
}
...
export function SampleData_todosFeed(numTodos, delay) {
    return DataSimulation_Records_Z46B86D6D(take_1(numTodos, SampleData_sampleTodos()), SampleData_updateTodo, 1, true, delay);
}
